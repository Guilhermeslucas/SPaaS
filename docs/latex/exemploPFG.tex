...
\begin{itemize}
  \item \textbf{Kubernetes\footnote{Orquestrador de containêres \emph{open source} https://kubernetes.io/}:} Orquestrador de containêres extremamente famoso nos dias de hoje, que tem como principal funcionalidade tornar mais simples, escalável e performática o deploy 
...
\begin{itemize}
  \item \textbf{\emph{Web Client}:} componente que roda no lado do cliente. Seguimos com o que foi proposto no início do projeto e explicado durante o procedimento, ou seja, todo o código do lado do cliente 
  ...
  \item \textbf{Flask \emph{Back End}:} responsável por receber todas as requisições que saem do front end. Funciona como um gateway. Também foi seguido o plano inicial de escrever todo o \emph{back end} com
  ...
  \item \textbf{Linux \emph{Workers}:} Componente que é responsável pela execução propriamente dita dos pedidos de trabalho. Com o auxílio do Celery, espera por pedidos entrarem na fila, e assim que isso acontece, executam. 
  ...
  O \emph{back end} somente precisava submeter uma mensagem com as definições para a execução de um trabalho na fila e algum \emph{worker} obteria essa requisição e faria o processamento necessário. Aqui vale uma observação 
  ...
  \item \textbf{resultsColleciton:} coleção de documentos no banco para armazenar os dados do resultado, armazena o id do processamento, dado e binário usado, além dos argumentos. 
  ...
  \item \textbf{seismic-results:} Outro Blob. Esse, responsável por armazenar os resultados dos processamentos. Todos os resultados são empacotados em um arquivo tar.gz\footnote{ utilitário de compressão de dados https://www.gzip.org/} e disponibilizados para download, 
