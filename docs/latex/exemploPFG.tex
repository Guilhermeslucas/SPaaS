...
\begin{itemize}
  ...
  Por exemplo: um dado sísmico que foi carregado pelo usuário.
  \item \textbf{\emph{Tool}:} ferramenta que é capaz de processar um dado do usuário. Por exemplo: uma ferramenta (CMP) que é capaz de realizar o 
  ...
  de intervenção manual ao término da execução de cada ferramenta. A Figura XX ilustra um fluxo de processamento.
  ...
  de download ou transformados em dados para processamento posterior pelo usuário. 
\end{itemize}
...
A Figura~\ref{fig:createScreen} deixa claro quais foram as informações que eram requeridas para se criar uma conta no sistema, que eram \emph{e-mail} senha e confirmação de senha.
...
as informações ilustradas na tela, eram o suficiente. Nessa tela os campos necessários eram o nome do processamento, qual o código seria executado, o comando necessário para rodar
a aplicação e o dado sísmico.
...
\begin{figure}[!h]
  \centering
  \includegraphics[scale=0.4]{tcc_flow.eps}
...
Assim, as atividades que um usuário deve realizar para que um processamento completo seja executado são os seguintes:
\begin{enumerate}
  ...
  \item \textbf{Submissão do dado sísmico:} uma vez que o usuário foi autenticado na plataforma, ele deverá submeter um dado sísmico que deseja para a execução do processamento. 
  \item \textbf{Submissão da \emph{Tool}:} além do dado, também é necessário a submissão de uma \emph{Tool} para o processamento.
  \item \textbf{Criação da tarefa:} com o dado sísmico e a \emph{Tool} submetidas, o usuário deve escolher qual o dado sísmico e \emph{Tool} vão compor o processamento.
  Além disso, o usuário também definirá quais os argumentos necessários.
  \item \textbf{Acompanhamento da tarefa:} ao criar a tarefa, o usuário poderá consultar se ela ainda está rodando e esperar até ficar pronta, para coletar os resultados.
  \item \textbf{Obtenção dos resultados:} ao final, o usuário poderá consultar os resultados que foram gerados. Todos os arquivos gerados estão reunidos em um arquivo
...
\begin{itemize}
  ...
  \begin{figure}[!h]
    ...
    \caption{Exemplo de um documento presente na coleção de \emph{Tools}}
    \label{fig:toolsCollection}
  \end{figure}

  \item \textbf{toolsCollection:} coleção dentro do banco que guarda informações sobre as \emph{Tools} que estão armazenadas. Essas informações são nome da \emph{Tool} e
  seus argumentos. Para cada argumento, é armazenado seu nome e descrição.
  Novos dados são inseridos nela quando uma nova \emph{Tool} é armazenada, quando
  um novo pedido de processamento é incluído e também quando uma \emph{Tool} é excluída.
  Um exemplo de descrição de uma \emph{Tool} que segue o modelo esperado está mostrado na Figura~\ref{fig:toolsCollection}.
  
  \begin{figure}[!h]
    \centering
    \includegraphics[scale=0.5]{results.eps}
    \caption{Exemplo de um documento presente na coleção de resultados}
    \label{fig:resultsCollection}
  \end{figure}

  \item \textbf{resultsColleciton:} coleção de documentos no banco que armazena os dados dos resultados, armazena o id do processamento, dado e \emph{Tool} usada, além dos argumentos. 
  Essa coleção é acessada após o término de cada processamento, para salvar as informações do resultado e também para listar resultados. Um exemplo de documento que segue
  o modelo esperado está ilustrado na Figura~\ref{fig:resultsCollection}.

  \begin{figure}[!h]
    \centering
    \includegraphics[scale=0.5]{users_model.eps}
    \caption{Exemplo de um documento presente da coleção de usuários}
  ...
  A coleção também é acessada para autenticação de um novo usuário. Para que todos esses processos sejam executados com sucesso, um exemplo de documento armazenado com as informações
  dos usuários está ilustrado na Figura~\ref{fig:usersCollection}.
...
Para executar um processamento sísmico, o usuário final não preciso de conhecimento de computação em nuvem, o que também era uma das premissas iniciais do projeto.
...
plano de implementar uma maneira do usuário conseguir analisar as imagens sísmicas geradas na própria interface \emph{web}.

\begin{thebibliography}{99}

\bibitem{UPP} A. Back and H. Lindén., {\it Cloud Computing Security: A Systematic Literature Review,} (2015).
